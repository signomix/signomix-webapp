<script>
    ...
    async function show(ctx) {
        try {
            ...
            if (
                ...
                apiUrl =
                    utils.getBackendUrl(location) + "/api/provider/v2/device/";
                promise = await sgxdata
                    .getData(dev, apiUrl, config, filter, $token, transform)
                    .then(function (data) {
    ...
    async function transform(widgetConfig, rawData) {
        ...
        let groupData = jsonData.queries["default"].group != null;
        console.log("group data " + groupData);
        console.log("reportInUse: " + reportInUse);
        if (reportInUse) {
            if (groupData) {
                return transformGroupData(widgetConfig, jsonData);
            } else {
    ...
    function transformData(widgetConfig, jsonData) {
        ...
        if (
            config.channel_translated !== null &&
            config.channel_translated !== undefined
        ) {
            translatedChannel = config.channel_translated;
        }
        if (
            config.channetTranslated != null &&
            config.channetTranslated !== undefined
        ) {
        ...
        if (widgetConfig.config !== null && widgetConfig.config !== undefined) {
            try {
                ...
                if (
                    customConfig.borderColors !== null &&
                    customConfig.borderColors !== undefined
                ) {
                    borderColors = customConfig.borderColors;
                }
                if (
                    customConfig.areaColors !== null &&
                    customConfig.areaColors !== undefined
                ) {
            ...
            for (var j = 0; j < jsonData[i].length; j++) {
                try {
                    tmpMeasures[j].push({
                        ...
                        y: getTransformedValue(
                            customConfig,
                            j,
                            jsonData[i][j]["value"],
                        ),
                    });
                    if (j == 0 && jsonData[i][j]["timestamp"] < dFirst) {
                        dFirst = jsonData[i][j]["timestamp"];
                    }
                    if (jsonData[i][j]["timestamp"] > dLast) {
                        dLast = jsonData[i][j]["timestamp"];
                    }
                } catch (err) {}
            }
        }
    ...
    function transformReportData(widgetConfig, jsonData) {
        ...
        if (
            config.channel_translated !== null &&
            config.channel_translated !== undefined &&
            config.channel_translated.trim().length > 0
        ) {
            translatedChannel = config.channel_translated;
        }
        if (
            config.channelTranslated !== null &&
            config.channelTranslated !== undefined &&
            config.channelTranslated.trim().length > 0
        ) {
        ...
        let dLast =
            jsonData.datasets[0].data[jsonData.datasets[0].size - 1].timestamp;
        for (var j = 0; j < jsonData.headers[0].columns.length; j++) {
            let measures = [];
            for (var i = 0; i < jsonData.datasets[0].size; i++) {
                measures.push({
                    x: new Date(
                        jsonData.datasets[0].data[i].timestamp,
                    ).toISOString(),
                    y: getTransformedValue(
                        customConfig,
                        j,
                        jsonData.datasets[0].data[i].values[j],
                    ),
                });
            }
            chartData.datasets.push({
                label: getChannelNameTranslated(
                    namesTranslated,
                    j,
                    jsonData.headers[0].columns[j],
                ),
                borderWidth: 1,
                data: measures,
                backgroundColor: areaColors[j % 7],
    ...
    function transformGroupData(widgetConfig, jsonData) {
        //console.log('chart report data', JSON.stringify(jsonData))
        //console.log('config.channel_translated', config.channel_translated)
        //console.log('widgetConfig.channel_translated', widgetConfig.channel_translated)
        ...
        let dFirst = jsonData.datasets[0].data[0].timestamp;
        let dSize = jsonData.datasets[0].data.length;
        let dLast =
            jsonData.datasets[0].data[dSize-1].timestamp;
        for (var j = 0; j < jsonData.datasets.length; j++) {
            let measures = [];
            for (var i = 0; i < jsonData.datasets[j].data.length; i++) {
                measures.push({
                    x: new Date(
                        jsonData.datasets[j].data[i].timestamp,
                    ).toISOString(),
                    y: getTransformedValue(
                        customConfig,
                        j,
                        jsonData.datasets[j].data[i].values[0],
                    ),
                });
            }
            chartData.datasets.push({
                label: jsonData.datasets[j].eui,
                borderWidth: 1,
                data: measures,
                backgroundColor: areaColors[j % 7],
                borderColor: borderColors[j % 7],
    ...
    function getTransformedValue(configuration, index, originalValue) {
        ...
        try {
            if (
                configuration != null &&
                configuration.multipliers != undefined &&
                configuration.multipliers != null
            ) {
                if (configuration.multipliers.length > index) {
                    resultValue =
                        resultValue * configuration.multipliers[index];
                }
            }
        } catch (err) {
