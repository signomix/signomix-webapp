    ...
    function handleSave() {
        ...
        if (errMessage != "") {
            toast.error(utils.getLabel(errMessage, labels, $language), {
                action: {
                    label: utils.getLabel("close", labels, $language),
                    onClick: () => {},
                },
            ...
            Promise.resolve(promise)
                ...
                .then((data) => {
                    ...
                    if (errTxt != null) {
                        ...
                        toast.error(
                            "1 " + utils.getLabel(errTxt, labels, $language),
                        );
                    } else {
                        //alert(data)
                        toast.success(data);
                ...
                .catch((error) => {
                    ...
                    toast.error(
                        "2 " +
                            utils.getLabel("error", labels, $language) +
                            ": " +
                            error.message,
                    );
                });
...
<form class="mb-2">
    ...
    <div class="row">
        <div class="col-md-12">
            <label for="input-decoder" class="form-label"
                >{utils.getLabel("decoder", labels, $language)}</label
            >
            <textarea
                class="form-control mb-1"
                id="input-decoder"
                rows="3"
                bind:value={decoderScript}
                {readonly}
            />
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <label for="input-processor" class="form-label"
                >{utils.getLabel("processor", labels, $language)}</label
            >
            <textarea
                class="form-control mb-1"
                id="input-processor"
                rows="3"
                bind:value={processorScript}
                {readonly}
            />
        </div>
    </div>
