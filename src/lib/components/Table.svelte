<script>
    ...
    export let selectAllowed = false; // can select rows
    export let selectMultiple = false; // can select multiple rows
    export let selectAll = false; // can select all rows
    ...
    <tbody class="table-group-divider">
            ...
            <tr>
                ...
                {#if index === editedRow}
                    ...
                    <td class="text-end">
                        <button
                            ...
                            on:click|preventDefault={() => saveRow(index)}
                ...
                {:else}
                    ...
                    <td class="text-end">
                        {#if updateAllowed}
                            <i
                                class="bi bi-pencil h5 me-2 link-dark"
                                title={utils.getLabel(
                                    "edit",
                                    labels,
                                    $language,
                                )}
                                on:click|preventDefault={() => onUpdate(index)}
                            ></i>
                        {/if}
                        {#if deleteAllowed}
                            <i
                                class="bi bi-trash3 h5 link-dark"
                                title={utils.getLabel(
                                    "delete",
                                    labels,
                                    $language,
        ...
        {#if createAllowed && !newRow}
            <tr>
                ...
                <td class="text-end">
                    <i
                        class="bi bi-plus-square h5 link-dark"
                        on:click={addRow}
                        title={utils.getLabel("add", labels, $language)}
                    ></i>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td class="text-end">
                    <button
                        type="button"
                        class="btn btn-outline-dark btn-sm"
                        on:click|preventDefault={saveNewRow}
                        >{utils.getLabel("save", labels, $language)}</button
                    >
                </td>
            </tr>{/if}
